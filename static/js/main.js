...
const LOCK_CLOSED_SVG = '<svg …></svg>';
const LOCK_OPEN_SVG   = '<svg …></svg>';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawGrid() {
    ...
    gridCanvas.style.width  = `${cols * sz}px`;
    gridCanvas.style.height = `${rows * sz}px`;
    canvas.style.width      = `${cols * sz}px`;
    canvas.style.height     = `${rows * sz}px`;
  ...
  // Redraw (including tile positioning + fixed opacity)
  function redraw() {
    drawGrid();
  ...
  controls.exportCSVBtn.addEventListener('click', () => {
    ...
    document.querySelectorAll('.tile').forEach(t => {
      rowsArr.push([
        +t.dataset.row * cols + +t.dataset.col + 1,
        t.dataset.name
      ]);
    });
  ...
  container.addEventListener('touchmove', e => {
    if (e.touches.length === 2) {
      ...
      setZoom(zoomLevel * (dist / startDist));
      startDist = dist;
    }
  });
  ...
  function swapTiles(t, col, row) {
    ...
    if (other && other !== t && other.dataset.locked === 'false') {
      ...
      other.dataset.col = ocol;
      other.dataset.row = orow;
      other.style.left   = `${ocol * sz}px`;
      other.style.top    = `${orow * sz}px`;
    }
    t.dataset.col = col;
    t.dataset.row = row;
    t.style.left  = `${col * sz}px`;
    t.style.top   = `${row * sz}px`;
  }
  ...
  function findFree(c0, r0, exclude = null) {
    ...
    canvas.querySelectorAll('.tile').forEach(t => {
      if (t !== exclude) occ.add(`${t.dataset.col},${t.dataset.row}`);
    });
